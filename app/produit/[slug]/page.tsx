          ...
          <div style={{
              ...
              {product.images && product.images.length > 1 && (
                <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1rem', flexWrap: 'wrap' }}>
                  {product.images.slice(1).map((img, i) => (
                    <div key={i} style={{ width: '80px', height: '80px', borderRadius: '10px', border: '1px solid #e5e7eb', overflow: 'hidden', background: '#ffffff' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem', marginBottom: '1.5rem' }}>
                <span style={{ fontFamily: 'var(--font-display)', fontSize: '2.5rem', fontWeight: 800, color: '#1a3a6b' }}>
                  {product.price.toLocaleString()}
                </span>
              ...
              <p style={{
                fontSize: '0.95rem', color: '#4b5563', lineHeight: 1.8,
              ...
              {/* SPECIFICATIONS */}
              {product.specifications && Object.keys(product.specifications).filter(k => k !== 'notes').length > 0 && (
                <div style={{ marginBottom: '1.5rem' }}>
                  <h3 style={{ fontSize: '0.8rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#6b7280', marginBottom: '0.75rem' }}>
                    Specifications
                  </h3>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
                    {Object.entries(product.specifications)
                      .filter(([k]) => k !== 'notes')
                      .map(([key, val]) => (
                        <div key={key} style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '0.6rem 0.75rem' }}>
                          <div style={{ fontSize: '0.68rem', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.15rem' }}>{key}</div>
                          <div style={{ fontSize: '0.85rem', fontWeight: 600, color: '#1f2937' }}>{String(val)}</div>
                        </div>
                      ))}
                  </div>
                </div>
                ...
                <div style={{ marginBottom: '2rem' }}>
                  ...
                  <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    {product.features.map((feature, i) => (
                      <li key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', fontSize: '0.9rem', color: '#374151' }}>
              ...
              {product.specifications?.notes && (
                <div style={{ background: '#fefce8', border: '1px solid #fde68a', borderRadius: '10px', padding: '1rem', marginBottom: '1.5rem', fontSize: '0.85rem', color: '#78350f', lineHeight: 1.6 }}>
                  {String(product.specifications.notes)}
                </div>
              )}
              ...
              <a
                ...
                style={{
                  ...
                  gap: '0.75rem', background: '#f5c518', color: '#1a1a2e',
                  padding: '1.1rem 2rem', borderRadius: '12px',
              ...
              <a
                ...
                style={{
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  gap: '0.75rem', background: '#1a3a6b', color: '#ffffff',
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginTop: '1.5rem' }}>
                {[
                  ...
                  <div key={item.text} style={{
                    ...
                    fontSize: '0.78rem', color: '#4b5563', fontWeight: 500,
                  }}>
          ...
          <section style={{ padding: '3rem 2rem 5rem', maxWidth: '1200px', margin: '0 auto' }}>
            <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: '3rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '1.25rem' }}>
                {related.map(p => (
                  <Link key={p.id} href={'/produit/' + p.slug} style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '14px', overflow: 'hidden', textDecoration: 'none', display: 'block' }}>
                    <div style={{ height: '160px', background: '#f9fafb', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
